...
from Read_HS_obj import reformat_obj
...
for tris in vs_list:
    ...
    new_p0_h = tranformation_matrix.dot(p_0_h)

    p_1 = tris[1]
    p_1_h = np.array([[p_1[0]], [p_1[1]], [p_1[2]], [1]])
    new_p1_h = tranformation_matrix.dot(p_1_h)

    p_2 = tris[2]
    p_2_h = np.array([[p_2[0]], [p_2[1]], [p_2[2]], [1]])
    new_p2_h = tranformation_matrix.dot(p_2_h)
